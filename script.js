...
settingsLogoutBtn: document.getElementById('settingsLogoutBtn'),
 emailInput: document.getElementById('settingsUsername'),
passwordInput: document.getElementById('settingsPassword'),
...
async function loadState() {
 ...
 if (state.sessionUser) {
  const { data, error } = await supabaseClient
    .from('mangas')
    .select('*')
    .eq('user_id', state.sessionUser);

  if (!error && data) {
    state.mangas = data.map(row => ({
      id: row.id,
      title: row.title,
      author: row.author,
      status: row.status,
...
async function saveState() {
  ...
  const rows = state.mangas.map(manga => ({
    id: manga.id,
    user_id: user,
    title: manga.title,
    author: manga.author,
    ...
    rating: manga.rating,
    last_read: manga.lastRead,
    release_date: manga.nextRelease,
...
function openVolumeDetail(volumeId) {
  ...
  dom.volumeNumberInput.value = volume.chapter || '';
  dom.volumeNameInput.value = volume.title || '';
  dom.volumeNotes.value = volume.notes || '';
  dom.volumeComments.value = volume.comments || '';
  dom.volumeCoverPreview.innerHTML = volume.cover 
    ? `<img src="${volume.cover}" alt="${volume.title || 'Volume cover'}" />`
    : `<div class="placeholder">No volume cover</div>`;
...
dom.settingsLogoutBtn.addEventListener('click', async () => {    try {
 ...
 dom.settingsBtn.addEventListener('click', () => {
  ...
  dom.volumeCoverFile.addEventListener('change', () => {
    ...
    handleVolumeCoverUpload(file, (base64) => {
     const volume = state.mangas.find(item => item.id === state.editingVolumeId);
if (!volume) return;

volume.cover = base64;
volume.chapter = dom.volumeNumberInput.value.trim() || volume.chapter;
volume.title = dom.volumeNameInput.value.trim() || volume.title;
volume.notes = dom.volumeNotes.value.trim();
volume.comments = dom.volumeComments.value.trim();

saveState();

const parent = state.mangas.find(item => item.id === volume.parentId);
if (parent) {
  renderVolumeGrid(parent);
}
...
showToast('Volume saved');
 ...
 dom.saveVolumeBtn.addEventListener('click', () => {
  ...
  volume.chapter = dom.volumeNumberInput.value.trim();
  volume.title = dom.volumeNameInput.value.trim();
  volume.notes = dom.volumeNotes.value.trim();
  volume.comments = dom.volumeComments.value.trim();
...
async function init() {
  ...
  if (data.session) {
    state.sessionUser = data.session.user.id;
  }
